...
(function($){
	$.fn.mailAutoComplete = function(options){
		...
		if(settings.autoClass && $("#mailListAppendCss").size() === 0){
			$('<style id="mailListAppendCss" type="text/css">.mailListBox{background:#fff; font:12px/20px Arial;border: 1px solid #aaa;border-bottom: none;}.mailListDefault{border-bottom:1px solid #aaa;padding:0 5px;cursor:pointer;white-space:nowrap;}.mailListFocus{padding:0 5px;cursor:pointer;white-space:nowrap;background:#369;color:white;}.mailListHlignt{color:red;}.mailListFocus .mailListHlignt{color:#fff;}</style>').appendTo($("head"));
		}
			...
			that.focus(function(){
				...
				$(this).keyup(function(e){
					...
					if(v.length > 0){
						...
						x.html($.createHtml(s, newArr, index)).css("left", 9);
						if(e.keyCode === 13){
							//回车
							if(index > -1 && index < newArr.length){
								//如果当前有激活列表
